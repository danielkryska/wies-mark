...
export const hasChildren = (category: ICategoryTree) => !!category && !!category.children && category.children.length > 0;
const hasSearchedChild = (category: ICategoryTree, categoryValue: string) => hasChildren(category) && category.children
  .some((childCategory: ICategoryTree) => childCategory.value === categoryValue);
const isParent = (category: ICategoryTree, categoryValue: string): boolean => hasSearchedChild(category, categoryValue);

const toChildren = (categories: ICategoryTree[]) => categories
...
export const flattenCategories = (...categories: ICategoryTree[]): ICategoryTree[] => categories
    .reduce((acc, category) => {
        acc.push(category);
        if (!category.isLeaf) {
...
export const isBelongingTo = (source: ICategoryTree, actual: ICategoryTree): boolean => isSearched(source, actual.value) 
